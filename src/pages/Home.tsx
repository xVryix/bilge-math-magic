...
import { CalendarCheck, Monitor, TrendingUp, Heart, ShieldCheck, FileText, Clock, CheckCircle, Star } from "lucide-react";
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      // Fetch approved 5-star reviews (first 4)
      ...
      if (fiveStarData) setReviews(fiveStarData);
      ...
      if (allData && allData.length > 0) {
        ...
        const avg = allData.reduce((sum, r) => sum + r.rating, 0) / total;
        setStats({ total, avg: Math.round(avg * 10) / 10 });
      }
    };
    fetchReviews();
  }, []);
          ...
          {hasReviews ? (
            <>
              <div className="mt-10 grid md:grid-cols-2 gap-6 text-left">
                {reviews.map((r, i) => (
                  <FadeIn key={r.id} delay={0.1 * (i + 1)}>
                    <div className="rounded-lg border bg-card p-6 h-full">
                      <div className="flex gap-0.5 mb-3">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star key={s} className={`w-4 h-4 ${s <= r.rating ? "fill-primary text-primary" : "text-border"}`} />
                        ))}
                      </div>
